...
export default (state, module) => {
  ...
  // checkIns.innerHTML = 'individual issues: ';
  ...
  if (module.weeks) {
    for (let i = 1; i <= module.weeks; i++) {
      ...
      checkIns.appendChild(
        linkButton(
          `week ${i}`,
          `https://github.com/${state.userName}/${state.repoName}/issues/?q=milestone%3A${module.repoName}+label%3Aindividual+label%3Aweek-${i}`
        )
      );
    }
  }
  ...
  const rollCalls = linkButton(
    'roll calls',
    `https://github.com/${state.userName}/${state.repoName}/issues/?q=milestone%3A${module.repoName}+label%3Aroll-call`
  );
  ...
  singletons.appendChild(document.createElement('br'));
  singletons.appendChild(rollCalls);
